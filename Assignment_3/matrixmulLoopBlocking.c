...
matrix_multiply(float C[N][N], const float A[N][N], const float B[N][N])
{
  for (size_t row = 0; row < N; row+=BLOCK) {
    for (size_t col = 0; col < N; col+=BLOCK) {
      ...
      for (size_t blockRow = 0; blockRow < N; blockRow++) {

        size_t max1 = (row + BLOCK < N) ? row + BLOCK : N;

        for (size_t blockCol = row; blockCol < max1; blockCol++) {
          float product = 0.0;

          size_t max2 = (col + BLOCK < N) ? col + BLOCK : N;
          for (size_t dot = col; dot < max2; dot++) {
            product += A[blockRow][dot] * B[dot][blockCol];
          }
          C[blockRow][blockCol] += product;
        }
